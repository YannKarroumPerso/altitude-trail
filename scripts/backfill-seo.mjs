#!/usr/bin/env node
// Backfill SEO lot 1 (auteur + liens internes + external refs + updatedAt).
// YouTube ignoré par défaut depuis la sandbox (proxy bloque googleapis).
...
if (proxyUrl) {
  try {
    ...
    console.log(`[backfill] proxy configure : ${proxyUrl}`);
  } catch (e) {
    console.warn(`[backfill] proxy config echoue : ${e.message}`);
  }
}
...
const noLlm = args.has("--no-llm");
const noYt = args.has("--no-yt");
...
async function main() {
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    try {
      ...
      const needsAuthor =
        !data.author ||
        /^\s*redaction\b/i.test(String(data.author).toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")) ||
      ...
      if (needsAuthor && data.categorySlug) {
        ...
        changed = true;
        console.log(`  + auteur attribue : ${picked.name}`);
      }
      ...
      if (!hasInternalLinks) {
        ...
        if (related.length) {
          ...
          changed = true;
          console.log(`  + ${related.length} liens internes : ${related.map((r) => r.slug).join(", ")}`);
        }
      }
      ...
      if (!noLlm && (!data.externalRefs || data.externalRefs.length === 0)) {
        ...
        if (proposed.length) {
          ...
          if (validated.length) {
            ...
            changed = true;
            console.log(`  + ${proposed.length} refs proposees, ${validated.length} validees`);
          }
        }
      }
